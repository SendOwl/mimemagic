...
RSpec.describe 'MimeMagic' do
  ...
  it 'should recognize by magic' do
    ...
    Dir['spec/fixtures/files/*'].each do |file|
      ...
      expect(actual).to eq(expected), "contents: #{file} is not #{expected}, it's #{actual}. This can happen if you add a file to spec/fixtures/files and the detection code relies on the file extension and your file does not have one"
      actual_bin = MimeMagic.by_magic(File.open(file, 'rb'))
      expect(actual_bin).to eq(expected), "file: #{file} is not #{expected}, it's #{actual_bin}. This can happen if you add a file to spec/fixtures/files and the detection code relies on the file extension and your file does not have one"
    end
  end
